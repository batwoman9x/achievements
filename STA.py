...
# Преобразование категориальных признаков (например, protocol_type, service, flag) в числовые
...
# Преобразование меток атак в бинарные (например, нормальный трафик и нет)
...
# Визуализация угроз во времени с использованием Plotly (на последнем фолде)
time = np.arange(len(y_pred))
threat_intensity = X_test[:, 0, 1]  # Пример использования одного признака как интенсивности угроз
